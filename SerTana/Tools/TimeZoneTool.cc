...
#include <SerTana/Tools/TimeZoneTool.h>
#include <SerTana/Tools/DateTool.h>
...
bool readTimeZoneFile(const char* zonefile, struct TimeZoneTool::Data* data)
{
  File f(zonefile);
  if (f.valid())
  {
...
const Localtime* findLocaltime(const TimeZoneTool::Data& data, Transition sentry, Comp comp)
{
...
TimeZoneTool::TimeZoneTool(const char* zonefile)
  : data_(new TimeZoneTool::Data)
{
...
TimeZoneTool::TimeZoneTool(int eastOfUtc, const char* name)
  : data_(new TimeZoneTool::Data)
{
...
struct tm TimeZoneTool::toLocalTime(time_t seconds) const
{
  struct tm localTime;
...
time_t TimeZoneTool::fromLocalTime(const struct tm& localTm) const
{
  assert(data_ != NULL);
  const Data& data(*data_);
...
struct tm TimeZoneTool::toUtcTime(time_t secondsSinceEpoch, bool yday)
{
  struct tm utc;
  ...
  DateTool date(days + DateTool::kJulianDayOf1970_01_01);
  DateTool::YearMonthDay ymd = date.yearMonthDay();
  ...
  if (yday)
  {
    DateTool startOfYear(ymd.year, 1, 1);
    utc.tm_yday = date.julianDayNumber() - startOfYear.julianDayNumber();
  }
  return utc;
}

time_t TimeZoneTool::fromUtcTime(const struct tm& utc)
{
  return fromUtcTime(utc.tm_year + 1900, utc.tm_mon + 1, utc.tm_mday,
                     utc.tm_hour, utc.tm_min, utc.tm_sec);
}

time_t TimeZoneTool::fromUtcTime(int year, int month, int day,
                             int hour, int minute, int seconds)
{
  DateTool date(year, month, day);
  int secondsInDay = hour * 3600 + minute * 60 + seconds;
  time_t days = date.julianDayNumber() - DateTool::kJulianDayOf1970_01_01;
