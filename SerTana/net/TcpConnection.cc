...
#include <SerTana/net/TcpConnection.h>

#include <SerTana/Tools/LoggingTool.h>
#include <SerTana/Tools/WeakCallback.h>
...
void SerTana::net::defaultConnectionCallback(const TcpConnectionPtr& conn)
{
...
void SerTana::net::defaultMessageCallback(const TcpConnectionPtr&,
                                        Buffer* buf,
                                        TimestampTool)
{
  buf->retrieveAll();
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
  ...
  if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) //这时候要确保没有开启写监听功能，
  {                                                                 //防止影响到别的应用程序对数据的写操作
...
void TcpConnection::handleRead(TimestampTool receiveTime)
{
  loop_->assertInLoopThread();
  int savedErrno = 0;
