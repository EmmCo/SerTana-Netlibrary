...
Connector::Connector(EventLoop* loop, const InetAddress& serverAddr)
  : loop_(loop),
    ...
    ConnectSocket_(sockets::createNonblockingOrDie(serverAddr_.family()))
{
  ConnectSocket_.setReuseAddr(true);
  ConnectSocket_.setReusePort(true);
  ConnectSocket_.setKeepAlive(true);


  LOG_DEBUG << "ctor[" << this << "]";
}

Connector::Connector(EventLoop* loop, const InetAddress& serverAddr,const InetAddress& localAddr)
  : loop_(loop),
    serverAddr_(serverAddr),
    ...
    state_(kDisconnected),
    retryDelayMs_(kInitRetryDelayMs),
    ConnectSocket_(sockets::createNonblockingOrDie(localAddr.family()))
{ 
  ConnectSocket_.setReuseAddr(true);
  ConnectSocket_.setReusePort(true);
  ConnectSocket_.setKeepAlive(true);
  ConnectSocket_.bindAddress(localAddr);
  LOG_DEBUG << "ctor[" << this << "]";
}
