...
TimerId EventLoop::runAt(const TimestampTool& time, const TimerCallback& cb)
{
...
{
  TimestampTool time(addTime(TimestampTool::now(), delay));
  return runAt(time, cb);
}

TimerId EventLoop::runEvery(double interval, const TimerCallback& cb)
{
  TimestampTool time(addTime(TimestampTool::now(), interval));
